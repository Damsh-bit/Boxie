...
import { isSlideKind, slideDefinitions, type BuyerPropsOf, type SlideKind, type ThemePropsOf } from './schemas'
...
export function readThemeConfig(input: unknown): ParsedThemeConfig {
  ...
  if (!result.success) throw new Error(`Configuración de temática inválida:\n${result.issues.join('\n')}`)
  return result.data
}
 ...
 */
export function parseBuyerContent(config: ParsedThemeConfig, input: unknown): ParseResult<BuyerContent> {
...
export function missingForLock(config: ParsedThemeConfig, content: BuyerContent): MissingField[] {
  ...
  if (!content.recipientName.trim()) missing.push({ path: 'recipientName', label: 'Para (destinatario)' })
...
export function collectAssetIds(content: BuyerContent): string[] {
  ...
  const visit = (value: unknown) => {
    ...
    if (typeof record.assetId === 'string' && Object.keys(record).length === 1) ids.add(record.assetId)
