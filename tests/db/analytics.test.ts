...
async function paidOrder(
  paymentId: string,
  opts: { amount?: number; discount?: number; couponId?: string } = {},
) {
...
describe('analítica del panel', () => {
  it('KPIs: facturación, ticket promedio y conversión del checkout', async () => {
    ...
    const [kpis] = await asAdmin(() =>
      db.query<Record<string, number>>(`select * from public.admin_kpis(${range})`),
    )
  ...
  it('ranking de temáticas y de cupones', async () => {
    ...
    const themes = await asAdmin(() =>
      db.query(
        `select theme_name, orders_paid, revenue_cents from public.admin_theme_ranking(${range})`,
      ),
    )
    expect(themes).toEqual([
      { theme_name: 'Tema de prueba', orders_paid: 2, revenue_cents: 2850000 },
    ])

    const coupons = await asAdmin(() =>
      db.query(`select code, uses, discount_cents from public.admin_coupon_ranking(${range})`),
    )
  ...
  it('ventas por día devuelve todos los días del período, con ceros', async () => {
    ...
    const days = await asAdmin(() =>
      db.query<{ orders_paid: number }>(
        `select * from public.admin_sales_by_day(now() - interval '3 days', now() + interval '1 minute')`,
      ),
    )
  ...
  it('cuenta las Boxies abiertas por el destinatario', async () => {
    ...
    const [kpis] = await asAdmin(() =>
      db.query<{ gifts_opened: number }>(`select gifts_opened from public.admin_kpis(${range})`),
    )
    expect(kpis!.gifts_opened).toBe(1)
    const [boxie] = await db.query<{ open_count: number }>(
      `select open_count from public.boxies where id = $1`,
      [boxie_id],
    )
